(:~
 ...
 :)
module namespace db = "x-http://basex.org/modules/db";
...
(:~
 : Opens the database <code>$db</code> and returns all document nodes.
 : The document nodes to be returned can be filtered with the <code>$path</code> argument.
 :
 : @error bxerr:BXDB0002 The addressed database does not exist or could not be opened.
 :)
...
(:~
 ...
 : The <a href="http://docs.basex.org/wiki/Node_Storage#PRE_Value">PRE value</a> provides very fast access to an existing database node, but it will change whenever a node with a smaller <i>pre</i> values is added to or deleted from a database.
 :
 : @error bxerr:BXDB0001 <code>$nodes</code> contains a node which is not stored in a database.
 :)
...
(:~
 ...
 :
 : @error bxerr:BXDB0002 The addressed database does not exist or could not be opened.
 : @error bxerr:BXDB0003 the database is not <i>persistent</i> (stored on disk).
...
(:~
 : Creates a new database with name <code>$db</code> and adds initial documents specified via <code>$inputs</code> to the specified <code>$paths</code> . An existing database will be overwritten.
 ...
 :
 : @error bxerr:FODC0002 <code>$inputs</code> points to an unknown resource.
 : @error bxerr:FOUP0001 <code>$inputs</code> is neither string nor a document node.
 : @error bxerr:BXDB0007 <code>$db</code> is opened by another process.
 : @error bxerr:BXDB0011 <code>$db</code> is not a <a href="http://docs.basex.org/wiki/Commands#Valid_Names">valid database name</a> .
 : @error bxerr:BXDB0012 two <code>db:create</code> statements with the same database name were specified.
...
(:~
 : Adds documents specified by <code>$input</code> to the database <code>$db</code> and the specified <code>$path</code> . A document with the same path may occur more than once in a database. If this is unwanted, <a href="#db:replace">db:replace</a> can be used.
 ...
 : The <code>$options</code> argument can be used to control parsing. The syntax is identical to the <a href="#db:create">db:create</a> function. Allowed options are all <a href="http://docs.basex.org/wiki/Options#Parsing">parsing</a> and <a href="http://docs.basex.org/wiki/Options#XML_Parsing">XML parsing</a> options.
 :
 : @error bxerr:BXDB0002 The addressed database does not exist or could not be opened.
 : @error bxerr:FODC0002 <code>$input</code> points to an unknown resource.
 : @error bxerr:FOUP0001 <code>$input</code> is neither string nor a document node.
 :)
...
(:~
 ...
 : @error bxerr:FOUP0001 <code>$input</code> is neither string nor a document node.
 :)
declare function db:add($db as xs:string, $input as item(), $path as xs:string, $options as item()) as empty-sequence() external;
...
(:~
 : Creates a copy of the database specified by <code>$db</code> to <code>$newname</code> .
 :
 : @error bxerr:BXDB0002 The addressed database does not exist or could not be opened.
 : @error bxerr:BXDB0011 Invalid database name.
 : @error bxerr:BXDB0016 Name of source and target database is equal.
 :)
declare function db:copy($db as xs:string, $newname as xs:string) as empty-sequence() external;

(:~
 : Renames the database specified by <code>$db</code> to <code>$newname</code> .
 :
 : @error bxerr:BXDB0002 The addressed database does not exist or could not be opened.
 : @error bxerr:BXDB0011 Invalid database name.
 : @error bxerr:BXDB0016 Name of source and target database is equal.
 :)
declare function db:alter($db as xs:string, $newname as xs:string) as empty-sequence() external;

(:~
 : Creates a backup of the database <code>$db</code> .
 :
 : @error bxerr:BXDB0002 The addressed database does not exist or could not be opened.
 : @error bxerr:BXDB0011 Invalid database name.
 :)
...
(:~
 : Drops all backups of the database with the specified <code>$name</code> . If the given <code>$name</code> points to a specific backup file, only this specific backup file is deleted.
 :
 : @error bxerr:BXDB0002 No backup file found.
 : @error bxerr:BXDB0011 Invalid database name.
 :)
declare function db:drop-backup($name as xs:string) as empty-sequence() external;

(:~
 : Restores the database with the specified <code>$name</code> . The <code>$name</code> may include the timestamp of the backup file.
 :
 : @error bxerr:BXDB0011 Invalid database name.
 : @error bxerr:BXDB0015 No backup found.
 :)
...
(:~
 ...
 :
 : @error bxerr:BXDB0002 The addressed database does not exist or could not be opened.
 : @error bxerr:BXDB0014 <code>$path</code> points to a directory.
 : @error bxerr:FODC0002 <code>$input</code> is a string representing a path, which cannot be read.
 : @error bxerr:FOUP0001 <code>$input</code> is neither a string nor a document node.
 :)
...
(:~
 ...
 : @error bxerr:FOUP0001 <code>$input</code> is neither a string nor a document node.
 :)
declare function db:replace($db as xs:string, $path as xs:string, $input as item(), $options as item()) as empty-sequence() external;
