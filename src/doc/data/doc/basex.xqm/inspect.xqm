...
(:~
 : Returns function items for all user-defined functions (both public and private) that are known in the current query context. If a <code>$uri</code> is specified, the addressed file will be compiled, its functions will be added to the query context and returned to the user.
 :)
...
(:~
 : Retrieves the string from the specified <code>$input</code> , parses it as XQuery module, and generates an element that describes its structure.
 :
 : @error xqdoc:FODC0002 the addressed resource cannot be retrieved.
 :)
declare function inspect:module($input as xs:string) as element(module) external;
...
(:~
 ...
 : @error xqdoc:FODC0002 the addressed resource cannot be retrieved.
 :)
declare function inspect:xqdoc($input as xs:string) as element(xqdoc:xqdoc) external;
