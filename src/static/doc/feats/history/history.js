...
// controllers
.controller("HistoryListCtrl", [  '$stateParams', '$scope','$location','ngDexie',
           function( $stateParams, $scope,$location,ngDexie) {
	$scope.sortopts=["id","timestamp"];
	...
	function update() {       
             ngDexie.getDb(function(db){
				db.log.toCollection().reverse().limit(20).offset($scope.params.start).toArray() 
